...
<script type="text/javascript">
		...
		*/
			...
			$.ajax({
				...
				success : function(json){
					...
					if(json.success){
						...
						html += '<h5 id="noteContent'+json.activityRemark.id+'">'+json.activityRemark.noteContent+'</h5>';
						html += '<font color="gray">市场活动</font> <font color="gray">-</font> <b>${activity.name}</b> <small style="color: gray;" id="small'+json.activityRemark.id+'"> '+ json.activityRemark.createTime +' 由'+ json.activityRemark.createBy +'</small>';
						html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
						html += '<a class="myHref" href="javascript:void(0);" onclick="popEditRemarkModal(\''+json.activityRemark.id+'\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: red;"></span></a>';
						html += '&nbsp;&nbsp;&nbsp;&nbsp;';
		...
		$("#updateBtn").click(function(){
			$.ajax({
				...
				success : function(json){
					//{"success" : true , "noteContent" : "","editTime" : "","editBy" : ""}
					//{"success" : false}
					if(json.success){
						$("#editActivityRemarkModal").modal("hide");
						//更新前端div当中的数据
						$("#noteContent" + $("#edit-id").val()).text(json.noteContent);
						$("#small" + $("#edit-id").val()).text(json.editTime + "由" + json.editBy);
					}else{
						alert("更新失败！");
					}
				}
	...
	function displayRemark(){
		...
		$.get(
				"workbench/activity/listRemark.do" , 
				{
					"activityId" : "${activity.id}"
				} , 
				function(json){
					//[{"id":"","noteContent":"","createTime":"","createBy":"","editTime":"","editBy":"","editFlag":"","activityId":""},{备注2}]
					var html = "";
					$.each(json , function(i , n){
						html += '<div id="'+n.id+'" class="remarkDiv" style="height: 60px;">';
						html += '<img title="zhangsan" src="image/user-thumbnail.png" style="width: 30px; height:30px;">';
						html += '<div style="position: relative; top: -40px; left: 40px;" >';
						html += '<h5 id="noteContent'+n.id+'">'+n.noteContent+'</h5>';
						...
						html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
						html += '<a class="myHref" href="javascript:void(0);" onclick="popEditRemarkModal(\''+n.id+'\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: red;"></span></a>';
						html += '&nbsp;&nbsp;&nbsp;&nbsp;';
	...
	function delById(id){ //这个id既是备注的id，又是备注div的id
		if(window.confirm("您确定要删除该备注信息吗？")){
			$.post(
	...
	function popEditRemarkModal(id){
		$("#edit-noteContent").val($("#noteContent" + id).text());
		$("#edit-id").val(id);
