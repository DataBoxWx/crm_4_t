...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#activityTbody2").on("click" , ":checkbox[name='id']" , function(){
			$("#firstChk").prop("checked" , $(":checkbox[name='id']").size() == $(":checkbox[name='id']:checked").size());
		});
		
		$("#searchByActivityName").keydown(function(event){ //event是一个变量名字，随意编写，它代表当前事件对象。
			if(event.keyCode == 13){
				//发送ajax get请求，查询符合条件的市场活动
				$.ajax({
					type : "get",
					cache : false,
					url : "workbench/clue/searchByActivityNameAndClueId.do",
					data : {
						"activityName" : $.trim(this.value),
						"clueId" : "${clue.id}"
					},
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input id="searchByActivityName" type="text" class="form-control" style="width: 300px;" placeholder="请输入市场活动名称，支持模糊查询">
					...
					<table id="activityTable" class="table table-hover" style="width: 900px; position: relative;top: 10px;">
						...
						<%--
							<tr>
								<td><input type="checkbox" name="id" value="xxx"/></td>
								<td>发传单</td>
								<td>2020-10-10</td>
								<td>2020-10-20</td>
								<td>zhangsan</td>
