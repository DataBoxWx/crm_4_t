...
crm-31
	...
	1、线索关联市场活动:
		...
		- 根据市场活动名称查询市场活动
			* 用户输入市场活动名称，然后回车进行查询，显示符合查询条件的市场活动列表。
			* 注意：线索已经关联过的市场活动，不能再次关联（不能查询出来，将这部分数据过滤掉）
			* jQuery防止事件起泡/冒泡/气泡：
				return false;
			
		- 用户选择对应的市场活动，点击关联，线索和市场活动关联。关联成功之后，关闭modal窗口，重新发送ajax请求查询“线索和市场活动关联列表”。
	
	2、支持拼音搜索（pinyin4j.jar）
	
crm-32
	
	1、通过clue/detail.jsp直接跳转到clue/convert.jsp
		两个jsp之间传递数据？
			detail.jsp --> uri?name=zhangsan&password=123
			convert.jsp --> ${param.name} ${param.password}
			
			${param.name}等同于哪些java代码？
				<%
	...
	2、用户点击转换：转换线索
		...
		注意：事务保证。同时成功或者同时失败！在一个service方法当中完成所有的操作。
...
crm-34
	...
	1、	用户在“交易列表”页面点击“创建”，跳转到“创建交易”页面：
		* 在“创建交易”页面中动态显示：所有者（用户）
		* 预计成交日期和下次联系时间添加日历控件
		* 阶段、类型、来源统一从缓存cache中取出并展示
		* 市场活动源和联系人名称固定
	
	2、用户在创建交易的时候，选择不同的阶段，显示不同的可能性。
		* 使用属性配置文件，该文件中描述了阶段与可能性的对应关系。
		* 服务器启动的时候，Listener读取资源文件，将资源文件当中的数据加载Map集合当中，将Map集合存储到application域中。
		* 前端页面上用户选择了不同的阶段，此时发生了：change事件，在回调函数当中设置可能性。
			
crm-35
	
	1、保存交易页面：
		填写客户名称的时候支持自动补全，这里使用bootstrap-typeahead插件
...
crm-37
	...
	1、查看交易明细。（只查看交易的详细信息）
		页面上需要展示：
			...
			* 联系人名称
		涉及到的表：
			tbl_tran t（交易的所有者一定有，交易的客户一定有，交易的市场活动源不一定有，交易的联系人不一定有）
			tbl_user u
			tbl_customer c
			tbl_activity a 
			tbl_contacts con
...
crm-38
	...
	1、查看交易明细：页面加载完毕之后，展示交易阶段历史。
		发送ajax get请求
			/crm/workbench/transaction/listHistory.do
			
crm-39	
	
	1、查看交易明细：展示阶段图标。
		* 为了图标能够全部动态生成，将图标修改为三种状态：
			- 已完成的
			- 正在进行的
			- 未进行的
		* 怎么动态呢？
			将属性配置文件当中的阶段前面添加编号，目的是让它可排序。
			
crm-40
	
	1、用户点击“交易阶段图标”之后，更新“交易明细”、“添加阶段历史”，这个版本不更新图标。
...
crm-42
	...
	1、显示交易统计图
		主要是统计“每个阶段stage”的交易数量比例。
		以公司为单位（以年为单位，以月为单位）
	
	2、sql语句的执行顺序：
		mysql: select (5)...from(1) ....where(2) ....group by(3)...having(4)...order by(6)...limit(7)
		oracle数据库不支持limit
		oracle比mysql语法严格很多。有很多时候在mysql中是可以执行的，但是在oracle中报错。编译报错。
	
crm-43
	
	1、使用junit4对crm项目当中的service进行测试，编写单元测试用例(junit test case)。
