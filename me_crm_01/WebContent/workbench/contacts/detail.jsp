...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#divRemark").on("mouseover",".remarkDiv",function(){
			$(this).children("div").children("div").show();
		});
		
		$("#divRemark").on("mouseout",".remarkDiv",function(){
			$(this).children("div").children("div").hide();
		});
		
		$("#divRemark").on("mouseover",".myHref",function(){
			$(this).children("span").css("color","red");
		});
		
		$("#divRemark").on("mouseout",".myHref",function(){
		...
		$("#select-activity").keydown(function(event){
			if(event.keyCode == 13){
				$.ajax({
					type : "get",
					url : "workbench/contacts/getActivityById.do",
					data : {
						"contactsId" : "${contacts.id}",
						"name" : $.trim($("#select-activity").val())
					},
					success : function(data){
						var html ='';
						$.each(data,function(i,n){
							html += '<tr>';
							html += '<td><input type="checkbox" name="bund-box" value="'+n.id+'"/></td>';
							html += '<td>'+n.name+'</td>';
		...
		$("#bundActivity").click(function(){
			var sendData = "contactsId=${contacts.id}";
			$.each($(":checkbox[name='bund-box']:checked"),function(i,n){
			...
			$.ajax({
				...
				data : sendData,
				beforeSend :function(){
					if($(":checkbox[name='bund-box']:checked").size() ==0){
						alert("请选择元素进行关联");
						return false;
					}
					return true;
		...
		$("#bund-qxbox").click(function(){
			$(":checkbox[name='bund-box']").prop("checked",this.checked);
		})
		//保存备注
			...
			$.ajax({
				...
				success : function(data){
					if(data.success){
						$("#remark").val("");
						var html ='';
						html += '<div id="div'+data.contactsRemark.id+'" class="remarkDiv" style="height: 60px;">';
						html += '<img title="zhangsan" src="image/user-thumbnail.png" style="width: 30px; height:30px;">';
						html += '<div style="position: relative; top: -40px; left: 40px;" >';
						html += '<h5 id=rem'+data.contactsRemark.id+'>'+data.contactsRemark.noteContent+'</h5>';
						html += '<font color="gray">联系人</font> <font color="gray">-</font> <b>${contacts.name}${contacts.appellation}-${contacts.customerId}</b> <small style="color: gray;" id="small-'+data.contactsRemark.id+'"> '+
						(data.contactsRemark.editFlag =="1" ? data.contactsRemark.editTime : data.contactsRemark.creatTime)+'由'+(data.contactsRemark.editFlag== "1" ? data.contactsRemark.editBy : data.contactsRemark.creatBy)+'</small>';
						html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
						html += '<a class="myHref" href="javascript:void(0);" onclick="editRemark(\''+data.contactsRemark.id+'\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #E6E6E6;"></span></a>';
						html += '&nbsp;&nbsp;&nbsp;&nbsp;';
						html += '<a class="myHref" href="javascript:void(0);" onclick="deleteRemark(\''+data.contactsRemark.id+'\')"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #E6E6E6;"></span></a>';
						html += '</div>';
						html += '</div>';
						html += '</div>';
		...
		$("#updateRemark").click(function(){
			$.ajax({
				...
				data : {
					"noteContent" : $.trim($("#edit-remark").val()),
					"id" : $("#h-remark").val(),
				},
				beforeSend : function(){
					if($.trim($("#edit-remark").val())==""){
						alert("备注不能为空");
						return false;
					}
					return true;
				...
				success : function(data){
					if(data.success){
						$("#rem" + data.contactsRemark.id).html(data.contactsRemark.noteContent);
						$("#samll" + data.contactsRemark.id).html(data.contactsRemark.editTime +' 由'+data.contactsRemark.editBy);
		...
		$("#unbundBtn").click(function(){
			$.post(
				...
				{
					"contactsId" : "${contacts.id}",
					"activityId" : $("#h-deleteId").val()
				},
				function(data){
					if(data.success){
						$("#bund-" +$("#h-deleteId").val() ).remove();
	...
	function deleteRemark(id){
		if(window.confirm("您确认删除吗?")){
			$.ajax({
				type : "post",
				url : "workbench/contacts/deleteReamrk.do",
				data : {
					"id" : id
				},
				success : function(data){
	...
	function displayRemark(){
		$.get(
			"workbench/contacts/displayRemark.do",
			{
				"contactsId" : "${contacts.id}"
			},
			function(data){
				...
				$.each(data,function(i,n){
					...
					html += '<img title="zhangsan" src="image/user-thumbnail.png" style="width: 30px; height:30px;">';
					html += '<div style="position: relative; top: -40px; left: 40px;" >';
					html += '<h5 id=rem'+n.id+'>'+n.noteContent+'</h5>';
					html += '<font color="gray">联系人</font> <font color="gray">-</font> <b>${contacts.name}${contacts.appellation}-${contacts.customerId}</b> <small style="color: gray;" id="small-'+n.id+'"> '+(n.editFlag =="1" ? n.editTime : n.creatTime)+'由'+(n.editFlag== "1" ? n.editBy : n.creatBy)+'</small>';
					html += '<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
	...
	function displayTransaction(){
		$.get(
			"workbench/contacts/displayTransaction.do",
			{
				"contactsid" : "${contacts.id}"
			},
			function(data){
				...
				$.each(data,function(i,n){
					html += '<tr>';
					html += '<td><a href="workbench/transaction/detail.do" style="text-decoration: none;">'+n.name+'</a></td>';
					html += '<td>'+n.money+'</td>';
					html += '<td>'+n.stage+'</td>';
					html += '<td>'+n.possiblity+'</td>';
					html += '<td>'+n.expectedTime+'</td>';
					html += '<td>'+n.type+'</td>';
					html += '<td><a href="javascript:void(0);" data-toggle="modal" data-target="#unbundModal" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>删除</a></td>';
					html += '</tr>';
				})
	...
	function displayActivity(){
		$.get(
			"workbench/contacts/displayActivity.do",
			{"contactsId" : "${contacts.id}"},
			function(data){
				var html ='';
				$.each(data,function(i,n){
					html += '<tr id="bund-'+n.id+'">';
					html += '<td><a href="workbench/activity/detail.do" style="text-decoration: none;">'+n.name+'</a></td>';
					html += '<td>'+n.startTime+'</td>';
					html += '<td>'+n.endTime+'</td>';
					html += '<td>'+n.owner+'</td>';
					html += '<td><a href="javascript:void(0);" onclick="deleteModal(\''+n.id+'\')" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
					html += '</tr>';
				})
...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 85%;">
			<div class="modal-content">
				<div class="modal-body">
					...
					<input type ="hidden" id="h-remark">
						<div class="form-group">
							<label for="edit-describe" class="col-sm-2 control-label">描述</label>
							<div class="col-sm-10" style="width: 81%;">
								<textarea class="form-control" rows="3" id="edit-remark">这是一条线索的描述信息</textarea>
							</div>
						</div>
	...
	<div class="modal fade" id="unbundActivityModal" role="dialog">
		<div class="modal-dialog" role="document" style="width: 30%;">
			<div class="modal-content">
				...
				<input type= "hidden" id= "h-deleteId">
	...
	<div class="modal fade" id="bundActivityModal" role="dialog">
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
					...
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" id="select-activity" class="form-control" style="width: 300px;" placeholder="请输入市场活动名称，支持模糊查询">
					...
					<table id="activityTable2" class="table table-hover" style="width: 900px; position: relative;top: 10px;">
						<thead>
							<tr style="color: #B3B3B3;">
								<td><input type="checkbox" id="bund-qxbox"/></td>
								<td>名称</td>
								<td>开始日期</td>
								<td>结束日期</td>
								<td>所有者</td>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
					<button type="button" class="btn btn-primary" id="bundActivity">关联</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		<div class="page-header">
			<h3>${contacts.name}${contacts.appellation}<small> - ${contacts.customerId}</small></h3>
		</div>
	...
	<div style="position: relative; top: -70px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 10px;">
			...
			<div style="width: 300px;position: relative; left: 650px; top: -60px;"><b>${contacts.name}${contacts.appellation}</b></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px;"></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px; left: 450px;"></div>
		</div>
		<div style="position: relative; left: 40px; height: 30px; top: 20px;">
			...
			<div style="width: 300px;position: relative; left: 650px; top: -60px;"><b>${contacts.phone}</b></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px;"></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px; left: 450px;"></div>
		</div>
		<div style="position: relative; left: 40px; height: 30px; top: 30px;">
			<div style="width: 300px; color: gray;">职位</div>
			<div style="width: 300px;position: relative; left: 200px; top: -20px;"><b>${contacts.position}</b></div>
		...
		<div style="position: relative; left: 40px; height: 30px; top: 40px;">
			...
			<div style="width: 500px;position: relative; left: 200px; top: -20px;"><b>${contacts.creatBy}&nbsp;&nbsp;</b><small style="font-size: 10px; color: gray;">${contacts.creatTime}</small></div>
		...
		<div style="position: relative; left: 40px; height: 30px; top: 80px;">
			...
			<div style="width: 300px;position: relative; left: 200px; top: -20px;"><b>&nbsp;${contacts.relationTimeNext}</b></div>
	...
	<!-- 备注 -->
	<div style="position: relative; top: 20px; left: 40px;" id="divRemark">
		<div class="page-header">
			<h4>备注</h4>
		</div>
